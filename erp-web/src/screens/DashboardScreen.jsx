...
const DashboardScreen = ({ ots = [], eliminarOT = [], solicitudes = [] }) => { // Recibe props para usar eliminarOT
    ...
    const [stats, setStats] = useState({ solicitudesNuevas: 0, totalHoras: 0 });

    // 1. FUNCIÓN MACRO: Calcular horas totales por puesto
    const calcularCargaHoraria = () => {
        const carga = {};
        // Cambiamos 'data.ots' por 'ots'
        ots?.forEach(ot => {
            ot.tareas?.forEach(t => {
...
const styles = {
    ...
    responsiveWrapper: {
        width: '100%',
        overflowX: 'auto', // Activa el scroll horizontal si la pantalla es pequeña
        paddingBottom: '60px', // Espacio para que no se corten sombras o tooltips
        borderRadius: '12px',
    },
    tableContainer: {
        background: 'white',
        ...
        minWidth: '1200px', // <--- ESTO EVITA QUE SE PIERDAN LOS BOTONES
    },
