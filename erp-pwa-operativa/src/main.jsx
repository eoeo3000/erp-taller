...
// entre "la app carga" y "pantalla en blanco" en terreno), y ese HTML apunta a los bundles de
// la build anterior, que también siguen en el caché — así que arranca la app vieja entera.
// Hasta acá nadie se enteraba nunca: sw.js no cambia entre deploys, así que el navegador no lo
...
const CLAVE_RECARGA = 'operativo.recargaPorVersion';
...
async function recargarSiHayVersionNueva() {
    try {
        ...
        // dejar la app usable (aunque desactualizada) antes que un bucle de recargas en terreno.
...
// Una PWA instalada casi no se cierra: se deja en segundo plano y se vuelve a ella entre
// trabajo y trabajo. Sin este chequeo al volver, la comprobación de arriba solo correría la
// primera vez que se abrió el teléfono.
